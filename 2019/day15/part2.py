...
def main():    
    ...
    with suppress(FileNotFoundError), open('data.pickle', 'rb') as f:
    ...
    while openset:
        # swap lists
        cloned_openset, openset = openset, []
        for current in cloned_openset:
            visited.add(current)
            for dir in delta.values():
                target = current + dir
                if target not in visited and target in corridors:
                    openset.append(target)
        
        print_map(map, position, visited)
        steps += 1
        print('Minutes:', steps)
        print("\033c")
